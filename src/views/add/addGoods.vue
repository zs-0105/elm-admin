			...
			<el-col :span="14" :offset="4">
				...
				<el-form :model="foodForm" :rules="foodrules" ref="foodForm" label-width="110px" class="form food_form">
					...
					<el-form-item label="上传食品图片">
						<el-upload class="avatar-uploader" :action="'https://elm.cangdu.org/v1/addimg/shop'"
					...
					<el-form-item label="食品特点">
						<el-select v-model="foodForm.attributes" multiple placeholder="请选择">
							<el-option v-for="item in attributes" :key="item.value" :label="item.label"
								:value="item.value">
							</el-option>
						</el-select>
					</el-form-item>
					...
					<el-row v-if="foodSpecs == 'one'">
						<el-form-item label="包装费">
							<el-input-number v-model="foodForm.specs[0].packing_fee" :min="0" :max="100">
							</el-input-number>
						</el-form-item>
					...
					<el-row v-else style="overflow: auto; text-align: center;">
						...
						<el-table :data="foodForm.specs" style="margin-bottom: 20px;"
							...
							<el-table-column label="操作">
								<template slot-scope="scope">
									<el-button size="small" type="danger" @click="handleDelete(scope.$index)">删除
									</el-button>
								</template>
							</el-table-column>
						</el-table>
...
<script>
import { getCategory, addCategory, addFood } from '@/api/food'
export default {
	data() {
		return {
			baseImgPath: process.env.VUE_APP_ImgUrl,
			restaurant_id: 1,
			categoryForm: {
				categoryList: [],
				categorySelect: '',
		...
		if (this.$route.query.restaurant_id) {
			...
			this.$msgbox({
				...
				beforeClose: (action, instance, done) => {
					if (action === 'confirm') {
						this.$router.push('/data/shopList');
						done();
					} else {
						this.$message({
							type: 'info',
